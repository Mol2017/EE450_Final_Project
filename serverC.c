...
int main(void)
{
	...
	/// Get ServerC's socket address (UDP)
	...
	if (ret != 0) 
	{
		fprintf(stderr, "ServerC getaddrinfo(): %s\n", gai_strerror(ret));
		exit(-1);
	}
	...
	fprintf(stdout, "The ServerC is up and running using UDP on port %s.\n", UDP_PORT);
	...
	{  
		...
		if (numbytes == -1) 
		{
			perror("ServerC recvfrom(): FAILED\n");
			exit(-1);
		}
		...
		if (TXLIST != op)
			fprintf(stdout, "The ServerC received a request from the Main Server.\n");
		char *username1 = NULL;
		char *username2 = NULL;
		if (CHECK_WALLET == op)
		{
		...
		else if (WRITE_LOGS == op)
		{
			...
			if (numbytes == -1) 
			{
				perror("ServerC sendto(): FAILED\n");
				exit(-1);
			}
			fprintf(stdout, "The ServerC finished sending the response to the Main Server.\n");
			continue;
		}
		else if (TXLIST == op) {}
		else
		{
			perror("ServerC: Invalid Service\n");
			exit(-1);
		}
