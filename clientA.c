/*
** clientA.c -- stream socket client A
*/
...
#define PORT        "25982"     // port number of main server client will connect to
#define MAXDATASIZE 10240       // max number of bytes we receive at once 

#define CHECK_WALLET 0          // operation declaration
#define TXCOINS      1
#define WRITE_LOGS 	 2
#define TXLIST 	     3
...
int main(int argc, char *argv[])
{
	...
	if (ret != 0) 
    {
		fprintf(stderr, "ClientA getaddrinfo(): %s\n", gai_strerror(ret));
		exit(-1);
	}
	...
	for(p = servinfo; p != NULL; p = p->ai_next) 
    {
        ...
        {
			perror("ClientA socket(): FAILED\n");
			continue;
		}

		if (connect(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
			perror("ClientA connect(): FAILED\n");
			close(sockfd);
			continue;
		}
		break;
	...
	if (p == NULL) {
		perror("ClientA failed to connect.\n");
		exit(-1);
	}
	freeaddrinfo(servinfo); // all done with this structure

    /// Print out booting up info
    fprintf(stdout, "The client A is up and running.\n");
    ...
    else if (op == TXLIST)
    {
        ...
        fprintf(stdout, "\"client A\" sent a sorted list request to the main server.\n");
    ...
    {
        perror("Invalid service in ClientA.\n");
	    exit(-1);
    }
