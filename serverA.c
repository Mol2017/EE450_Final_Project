...
int main(void)
{
	...
	for(p = servinfo; p != NULL; p = p->ai_next) 
	{
		...
		{
			perror("ServerA socket(): FAILED\n");
			continue;
		}

		if (bind(sockfd, p->ai_addr, p->ai_addrlen) == -1) {
			close(sockfd);
			perror("ServerA bind(): FAILED\n");
			continue;
		}
		break;
	}
	...
	if (p == NULL)  
	{
		perror("ServerA failed to bind.\n");
		exit(-1);
	}

	/// Print out booting up info
	fprintf(stdout, "The ServerA is up and running using UDP on port %s.\n", UDP_PORT);
	...
	{  
		...
		if (numbytes == -1) 
		{
			perror("ServerA recvfrom(): FAILED\n");
			exit(-1);
		}
		...
		int op = recv_buffer[0] - '0'; // recv_buffer e.g. 1\tJohn
		if (TXLIST != op)
			fprintf(stdout, "The ServerA received a request from the Main Server.\n");
		char *username1 = NULL;
		char *username2 = NULL;
		if (CHECK_WALLET == op)
		{
		...
		else if (WRITE_LOGS == op)
		{
			...
			if (numbytes == -1) 
			{
				perror("ServerA sendto(): FAILED\n");
				exit(-1);
			}
			fprintf(stdout, "The ServerA finished sending the response to the Main Server.\n");
			continue;
		}
		else if (TXLIST == op) {}
		else
		{
			perror("ServerA: Invalid Service\n");
			exit(-1);
		}
